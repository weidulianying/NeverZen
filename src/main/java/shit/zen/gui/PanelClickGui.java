...
import shit.zen.gui.panel.setting.PasswordSettingRenderer;
...
extends Screen {
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (this.profileWidget.isPopupOpen() && this.profileWidget.getSettingsPopup().handleKeyPress(keyCode, scanCode, modifiers)) {
            return true;
        }
        if (this.searchActive) {
            if (keyCode == 256) {
        ...
        if (PasswordSettingRenderer.onKeyPress(keyCode)) {
            return true;
        }
    ...
    public boolean charTyped(char c, int modifiers) {
        if (this.profileWidget.isPopupOpen() && this.profileWidget.getSettingsPopup().handleCharTyped(c, modifiers)) {
            return true;
        }
        ...
        if (PasswordSettingRenderer.onCharTyped(c)) {
            return true;
        }
